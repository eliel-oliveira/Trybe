...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  ...
  it("TESTE 2: Verifique se 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são somente `food` e `drink`.", () => {
    expect(createMenu(pedido).fetchMenu()).toEqual({
      food: {},
      drink: {}
    });
  });
  ...
  it("TESTE 5: Verifique se, ao chamar uma função associada à chave `order` no objeto retornado passando uma string como parâmetro , tal string é adicionada", () => {
    expect(createMenu().order()('coxinha')).toEqual(expect.arrayContaining(['coxinha']));
  });
  ...
  it('Teste 6: Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array objetoRetornado.consumpition contém os itens pedidos', () => {
    createMenu().consumption.splice('coxinha') //limpa teste anterior
    expect(createMenu().order()('coxinha')).toEqual(expect.arrayContaining(['coxinha']));
    expect(createMenu().order()('agua')).toEqual(expect.arrayContaining(["coxinha", "agua"]));
    expect(createMenu().order()('sopa')).toEqual(expect.arrayContaining(["coxinha", "agua", "sopa"]));
    expect(createMenu().order()('sashimi')).toEqual(expect.arrayContaining(["coxinha", "agua", "sopa", "sashimi"]));
  ...
  // ```
  it('TESTE 7: Verifique se a função order aceita que pedidos repetidos sejam acrescidos a consumition', () => {
    expect(createMenu().order()('agua')).toEqual(expect.arrayContaining(["coxinha", "agua", "sopa", "sashimi", "agua"]));
  });
  ...
  it('TESTE 8: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`.', () => {
    createMenu().consumption.splice('agua') //limpa testes anteriores
    createMenu().consumption.splice('sopa') //limpa testes anteriores
    createMenu().consumption.splice('sashimi') //limpa testes anteriores
    expect(createMenu().order()('café')).toEqual(expect.arrayContaining(['café']));
    expect(createMenu().order()('coxinha')).toEqual(expect.arrayContaining(['café', 'coxinha']));
    expect(createMenu().order()('coxinha')).toEqual(expect.arrayContaining(['café', 'coxinha', 'coxinha']));
    expect(createMenu().pay()()).toBe(12.1);
  });
